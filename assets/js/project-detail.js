(() => {
  ...
  const getExternalImages = () => {
    if (!externalImagesPromise) {
      externalImagesPromise = fetch(`data/project-external-images.json?v=${Date.now()}`, { cache: 'no-store' })
        ...
        .then(data => {
          const index = new Map();
          Object.entries(data || {}).forEach(([title, record]) => {
  ...
  const getGalleryItems = (project, externalIndex) => {
    ...
    // Deux photos locales ou plus signifient que le client a déjà constitué sa galerie.
    // Dans ce cas, les images trouvées en ligne ne sont jamais ajoutées.
