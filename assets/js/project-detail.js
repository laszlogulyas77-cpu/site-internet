(() => {
  ...
  modal.innerHTML = `
    ...
    <div class="project-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="project-detail-title" aria-describedby="project-detail-description">
      ...
      <div class="project-detail-media" data-project-detail-media>
        ...
        <button class="project-gallery-expand" type="button" aria-label="Agrandir la galerie" aria-pressed="false" data-project-gallery-expand>
          <span class="project-gallery-expand-icon" aria-hidden="true">⛶</span>
          <span class="project-gallery-expand-label" data-project-gallery-expand-label>Agrandir</span>
        </button>
        ...
        <div class="project-gallery-footer" data-project-gallery-footer hidden>
          <div class="project-gallery-dots" data-project-gallery-dots aria-label="Miniatures des photos"></div>
  ...
  const dialog = modal.querySelector('.project-detail-dialog');
  const media = modal.querySelector('[data-project-detail-media]');
  const image = modal.querySelector('[data-project-detail-image]');
  const expandButton = modal.querySelector('[data-project-gallery-expand]');
  ...
  const enhanceCard = card => {
    if (card.dataset.projectDetailReady === 'true') return;
  ...
  const applyExternalCardFallback = async card => {
    ...
    try {
      ...
      const cardImage = card.querySelector(':scope > img') || card.querySelector('img');
      if (!cardImage) return;
      let candidateIndex = 0;
      const tryNextImage = () => {
        if (candidateIndex >= candidates.length) {
          cardImage.onerror = null;
          cardImage.src = project.image || placeholderImage;
          return;
        }
        cardImage.src = candidates[candidateIndex++];
      };
      cardImage.loading = 'lazy';
      cardImage.decoding = 'async';
      cardImage.onerror = tryNextImage;
      cardImage.alt = project.alt || project.title || 'Réalisation SERILEC';
      tryNextImage();
    } catch (error) {
  ...
  const setGalleryExpanded = expanded => {
    dialog.classList.toggle('is-gallery-expanded', expanded);
    expandButton.setAttribute('aria-pressed', String(expanded));
    expandButton.setAttribute('aria-label', expanded ? 'Réduire la galerie' : 'Agrandir la galerie');
    const label = modal.querySelector('[data-project-gallery-expand-label]');
  ...
  const showGalleryImage = (index, failedAttempts = 0) => {
    ...
    media.style.setProperty('--project-gallery-bg', `url("${escapeCssUrl(item.url)}")`);
    ...
    image.onerror = () => {
      ...
      media.style.setProperty('--project-gallery-bg', `url("${escapeCssUrl(placeholderImage)}")`);
    ...
    modal.querySelectorAll('[data-project-gallery-index]').forEach(thumb => {
      const active = Number(thumb.dataset.projectGalleryIndex) === galleryIndex;
  ...
  const setupGallery = (project, externalIndex) => {
    ...
    const thumbs = modal.querySelector('[data-project-gallery-dots]');
    media.classList.toggle('has-gallery', hasGallery);
    prev.hidden = !hasGallery;
    next.hidden = !hasGallery;
    footer.hidden = !hasGallery;
    thumbs.innerHTML = hasGallery
      ? galleryItems.map((item, index) => `<button class="project-gallery-thumb${index === 0 ? ' active' : ''}" type="button" data-project-gallery-index="${index}" aria-label="Voir la photo ${index + 1}" aria-current="${index === 0 ? 'true' : 'false'}"><img src="${escapeHtml(item.url)}" alt="" loading="lazy" decoding="async"></button>`).join('')
      : '';
    thumbs.querySelectorAll('img').forEach(thumbImage => {
      thumbImage.addEventListener('error', () => thumbImage.closest('.project-gallery-thumb')?.classList.add('is-broken'), { once: true });
    });
    showGalleryImage(0);
  };
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-project-gallery-expand]')) {
      setGalleryExpanded(!dialog.classList.contains('is-gallery-expanded'));
      return;
    }
    ...
    const thumb = event.target.closest('[data-project-gallery-index]');
    if (thumb) {
      showGalleryImage(Number(thumb.dataset.projectGalleryIndex));
      return;
    }
  ...
  document.addEventListener('keydown', event => {
    if (!modal.hidden) {
      if (event.key === 'Escape') {
        if (dialog.classList.contains('is-gallery-expanded')) setGalleryExpanded(false);
        else closeProject();
        return;
      }
